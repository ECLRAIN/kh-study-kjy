...
-- 날짜 조건 설정
-- 문자열처럼 사용할 수도 있고, 계산도 가능, 범위 표현도 가능
...
select * from product 
where 
    extract(month from made) between 6 and 8;
    
select * from product where 
made like '%/06/%' or made like '%/07/%' or made like '%/08/%';

select * from product where
to_char(made, 'mm') in ('06', '07', '08');

select * from product where
regexp_like(to_char(made, 'mm'), '(06|07|08)');

-- 2019년 6월 1일부터 2019년 8월 31일까지 조회
-- (주의) 시간을 입력하지 않으면 00시 00분 00초로 설정됨
select * from product 
where made >= to_date('2019-06-01 00:00:00', 'yyyy-mm-dd hh24:mi:ss') 
        and made <= to_date('2019-08-31 23:59:59', 'yyyy-mm-dd hh24:mi:ss');

select * from product 
where made 
    between to_date('2019-06-01 00:00:00', 'yyyy-mm-dd hh24:mi:ss') 
...
select * from product order by name asc where price <= 2000;
