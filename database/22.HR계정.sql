...
select JH.*, (end_date - start_date + 1) 근무일수 from job_history JH
where (end_date-start_date+1) = (select max(end_date - start_date + 1) from job_history);

select * from (
    select TMP.*, rownum rn from (
        select JH.*, (end_date - start_date + 1) 근무일수 
        from job_history JH order by 근무일수 desc
    )TMP
) where rn = 1;
...
select 
    job_id, avg(salary) 평균, max(salary) 최대, min(salary) 최소 
from employees
group by job_id;
